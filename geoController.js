...
exports.getData = async (req, res) => {
    try {
        ...
        if (result.candidates && result.candidates.length > 0) {
            ...
            for (var candidate of result.candidates) {
                const candidateArea = candidate.address.toLowerCase()
                if (candidateArea.includes(inputArea)) {
                    location.latitude = candidate.location.y.toString()
                    location.longitude = candidate.location.x.toString()
                    break 
                }
            }
        }
